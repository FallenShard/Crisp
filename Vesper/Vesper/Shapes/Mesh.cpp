...
namespace crisp
{
    Mesh::Mesh(const VariantMap& params)
        : m_mesh(loadTriangleMesh(params.get<std::string>("filename")))
    {
    ...
    void Mesh::fillIntersection(unsigned int triangleId, const Ray3& /*ray*/, Intersection& its) const
    {
        ...
        its.shFrame = !m_mesh.getNormals().empty()
            ? CoordinateFrame(m_mesh.interpolateNormal(triangleId, barycentric))
            : its.geoFrame;
    ...
    void Mesh::sampleSurface(Shape::Sample& shapeSample, Sampler& sampler) const
    {
        ...
        shapeSample.n = !m_mesh.getNormals().empty()
