...
namespace
{
auto logger = createLoggerMt("LuaPipelineBuilder");
}
...
void LuaPipelineBuilder::readViewportState(Renderer* renderer, const VulkanRenderPass& renderPass)
{
    ...
    {
        m_builder.setViewport(renderer->getDefaultViewport()).addDynamicState(VK_DYNAMIC_STATE_VIEWPORT);
        m_builder.setScissor(renderer->getDefaultScissor()).addDynamicState(VK_DYNAMIC_STATE_SCISSOR);
        return;
    }
    ...
    if (auto scissor = m_config.get<std::vector<int32_t>>("scissor"); scissor && scissor.value().size() == 4)
    {
        ...
        m_builder.setScissor(VkRect2D{ { vec[0], vec[1] }, { static_cast<uint32_t>(vec[2]),
    static_cast<uint32_t>(vec[3]) } });
    }
