    ...
    uint32_t numLayers)
{
    ...
    renderer->enqueueResourceUpdate(
        ...
        {
            image.transitionLayout(cmdBuffer, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, layerIdx, numLayers,
                VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);
            image.copyFrom(cmdBuffer, *stagingBuffer, layerIdx, numLayers);
            image.transitionLayout(cmdBuffer, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, layerIdx, numLayers,
...
std::unique_ptr<VulkanImage> createTexture(Renderer* renderer, const Image& image, VkFormat format)
{
    ...
    renderer->enqueueResourceUpdate(
        ...
        {
            image->transitionLayout(cmdBuffer, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 0, 1,
                VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);
            ...
            image->transitionLayout(cmdBuffer, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, mipRange,
                VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
        });
...
std::unique_ptr<VulkanImage> createTexture(Renderer* renderer, const std::string& filename, const VkFormat format,
    const FlipOnLoad flipOnLoad)
{
    return createTexture(renderer,
        loadImage(renderer->getResourcesPath() / "Textures" / filename, 4, flipOnLoad).unwrap(), format);
}
...
std::unique_ptr<VulkanImage> createEnvironmentMap(Renderer* renderer, const std::string& filename,
    const VkFormat format, const FlipOnLoad flipOnLoad)
{
    return createTexture(renderer,
...
std::unique_ptr<VulkanImage> createMipmapCubeMap(Renderer* renderer, const uint32_t width, const uint32_t height,
