    ...
    : m_renderer(renderer)
{
    ...
    for (auto& perFrameCtx : m_secondaryCommandBuffers)
    {
        ...
        {
            ctx.pool = std::make_unique<VulkanCommandPool>(m_renderer->getDevice().getGeneralQueue().createCommandPool(
...
void RenderGraph::addDependency(std::string sourcePass, std::string destinationPass,
    RenderGraph::DependencyCallback callback)
{
    m_nodes.at(sourcePass)->dependencies[destinationPass] = callback;
}

void RenderGraph::addRenderTargetLayoutTransition(const std::string& sourcePass, const std::string& destinationPass,
...
{
    m_threadPool.parallelJob(renderNodes.size(),
...
void RenderGraph::executeRenderPass(VulkanCommandBuffer& cmdBuffer, uint32_t virtualFrameIndex, const Node& node) const
{
    ...
    for (uint32_t subpassIndex = 0; subpassIndex < node.renderPass->getNumSubpasses(); subpassIndex++)
    {
        ...
        if (useSecondaries)
        {
            m_threadPool.parallelJob(node.commands[subpassIndex].size(),
