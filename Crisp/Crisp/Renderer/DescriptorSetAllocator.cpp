...
DescriptorSetAllocator::DescriptorSetAllocator(VulkanDevice& device,
    const std::vector<std::vector<VkDescriptorSetLayoutBinding>>& setBindings,
    const std::vector<uint32_t>& numCopiesPerSet, VkDescriptorPoolCreateFlags flags)
    : m_device(&device)
{
...
VkDescriptorSet DescriptorSetAllocator::DescriptorPool::allocate(VkDevice device, VkDescriptorSetLayout setLayout,
    const std::vector<VkDescriptorSetLayoutBinding>& setBindings)
{
    currentAllocations++;
