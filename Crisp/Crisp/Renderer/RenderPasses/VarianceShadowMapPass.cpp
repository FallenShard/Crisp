...
void VarianceShadowMapPass::createResources()
{
    ...
    m_renderTargets[0] =
        std::make_unique<VulkanImage>(m_renderer->getDevice(), extent, numLayers, 1, VK_FORMAT_R32G32_SFLOAT,
            VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_IMAGE_ASPECT_COLOR_BIT, 0);
    m_renderTargets[1] = std::make_unique<VulkanImage>(m_renderer->getDevice(), extent, numLayers, 1,
        VK_FORMAT_D32_SFLOAT, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_IMAGE_ASPECT_DEPTH_BIT, 0);
    ...
    for (uint32_t i = 0; i < numLayers; i++)
    {
        ...
        m_framebuffers[i] =
            std::make_unique<VulkanFramebuffer>(m_renderer->getDevice(), m_handle, m_renderArea, attachmentViews);
    }
}
} // namespace crisp