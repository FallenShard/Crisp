...
void ForwardLightingPass::createResources(Renderer& renderer)
{
    if (m_defaultSampleCount == VK_SAMPLE_COUNT_1_BIT)
    {
    ...
    else
    {
        ...
        m_renderTargets[0] = std::make_unique<VulkanImage>(renderer.getDevice(), createInfo, VK_IMAGE_ASPECT_COLOR_BIT);
        m_renderTargets[1] = std::make_unique<VulkanImage>(renderer.getDevice(), depthInfo, VK_IMAGE_ASPECT_DEPTH_BIT);
        m_renderTargets[2] =
            std::make_unique<VulkanImage>(renderer.getDevice(), resolveInfo, VK_IMAGE_ASPECT_COLOR_BIT);
