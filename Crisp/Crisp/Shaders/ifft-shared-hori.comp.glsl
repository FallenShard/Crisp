...
// Whole-row IFFT in shared memory: one workgroup per row, N/2 threads cooperating on that row.
// Replaces the separate bit-reverse + logN per-stage dispatches with one dispatch that loads the
// row bit-reversed, runs all logN butterfly stages with barrier() between them, and stores once.
...
void main()
{
    const int row = int(gl_WorkGroupID.y);
    const int tid = int(gl_LocalInvocationID.x);
    const int half_ = N / 2;

    data[tid] = imageLoad(srcImg, ivec2(int(reverseBits(uint(tid), logN)), row)).xy;
    data[tid + half_] = imageLoad(srcImg, ivec2(int(reverseBits(uint(tid + half_), logN)), row)).xy;
    barrier();
    ...
    imageStore(dstImg, ivec2(tid, row), vec4(data[tid], 0.0, 0.0));
    imageStore(dstImg, ivec2(tid + half_, row), vec4(data[tid + half_], 0.0, 0.0));
}
