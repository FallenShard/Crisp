...
void LutTransmittanceParamsToUv(
    in AtmosphereParams atmosphere, in float viewHeight, in float viewZenithCosAngle, out vec2 uv) {
    float H = sqrt(
...
// vec3 GetMultipleScattering(AtmosphereParameters Atmosphere, vec3 scattering, vec3 extinction, vec3 worlPos, float
// viewZenithCosAngle)
// {
// 	vec2 uv = clamp(vec2(viewZenithCosAngle*0.5f + 0.5f, (length(worlPos) - Atmosphere.BottomRadius) /
// (Atmosphere.TopRadius - Atmosphere.BottomRadius)), vec2(0), vec2(1)); 	uv = vec2(fromUnitToSubUvs(uv.x,
// MultiScatteringLUTRes), fromUnitToSubUvs(uv.y, MultiScatteringLUTRes));
