...
void main() {
    ...
    for (uint i = 0; i < lightIndexData[1]; ++i) {
        ...
        vec3 Le = evalPointLightRadiance(
            pointLights[idx].position.xyz, pointLights[idx].spectrum.xyz, pointLights[idx].params.r, eyeL);
        acc += max(dot(eyeN, eyeL), 0.0f) * Le;
    }

    //     //vec3 Le = evalPointLightRadiance(pointLights[0].position.xyz, pointLights[0].spectrum.xyz,
    //     pointLights[0].params.r, eyeL);
    //     //acc = max(dot(eyeN, eyeL), 0.0f) * Le;
    // //
    // //
    // //
