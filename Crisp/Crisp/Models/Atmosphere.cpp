...
namespace crisp
{
    ...
    addAtmosphereRenderPasses(RenderGraph& renderGraph, Renderer& renderer, ResourceContext& resourceContext, const std::string& dependentPass)
    {
        ...
        resourceContext.createUniformBuffer("atmosphere", sizeof(SkyAtmosphereConstantBufferStructure), BufferUpdatePolicy::PerFrame);
        resourceContext.createUniformBuffer("atmosphereCommon", sizeof(CommonConstantBufferStructure), BufferUpdatePolicy::PerFrame);

        // Transmittance lookup
        renderGraph.addRenderPass("TransLUTPass", std::make_unique<TransmittanceLutPass>(&renderer));
        auto transLutPipeline = resourceContext.createPipeline("transLut", "SkyTransLut.lua", renderGraph.getRenderPass("TransLUTPass"), 0);
        ...
        resourceContext.addImage("multiScatTex", std::make_unique<VulkanImage>(renderer.getDevice(), createInfo, VK_IMAGE_ASPECT_COLOR_BIT));
        resourceContext.addImageView("multiScatTexView0", resourceContext.getImage("multiScatTex")->createView(VK_IMAGE_VIEW_TYPE_2D, 0, 1));
        resourceContext.addImageView("multiScatTexView1", resourceContext.getImage("multiScatTex")->createView(VK_IMAGE_VIEW_TYPE_2D, 1, 1));

        renderer.enqueueResourceUpdate([tex = resourceContext.getImage("multiScatTex")](VkCommandBuffer cmdBuffer) {
            tex->transitionLayout(cmdBuffer, VK_IMAGE_LAYOUT_GENERAL, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT);
        });
        ...
        std::vector<VulkanImageView*> views{resourceContext.getImageView("multiScatTexView0"), resourceContext.getImageView("multiScatTexView1")};
        multiScatPass.material->writeDescriptor(1, 0, views, nullptr, VK_IMAGE_LAYOUT_GENERAL);
        multiScatPass.material->writeDescriptor(1, 1, renderGraph.getRenderPass("TransLUTPass"), 0, resourceContext.getSampler("linearClamp"));
        ...
        renderGraph.addDependency("MultiScatPass", "SkyViewLUTPass", [tex = resourceContext.getImage("multiScatTex")](const VulkanRenderPass&, VkCommandBuffer cmdBuffer, uint32_t frameIndex)
        {
        ...
        // Sky View LUT
        auto skyViewLutPipeline = resourceContext.createPipeline("skyViewLut", "SkyViewLut.lua", renderGraph.getRenderPass("SkyViewLUTPass"), 0);
        ...
        skyViewLutMaterial->writeDescriptor(1, 0, renderGraph.getRenderPass("TransLUTPass"), 0, resourceContext.getSampler("linearClamp"));
        skyViewLutMaterial->writeDescriptor(1, 1, views, resourceContext.getSampler("linearClamp"), VK_IMAGE_LAYOUT_GENERAL);
        ...
        cameraVolumesMaterial->writeDescriptor(1, 0, renderGraph.getRenderPass("TransLUTPass"), 0, resourceContext.getSampler("linearClamp"));
        cameraVolumesMaterial->writeDescriptor(1, 1, views, resourceContext.getSampler("linearClamp"), VK_IMAGE_LAYOUT_GENERAL);
        ...
        auto skyCameraVolumesNode = renderNodes.emplace("skyCameraVolumesNode", std::make_unique<RenderNode>()).first->second.get();
        ...
        auto rayMarchingPipeline = resourceContext.createPipeline("rayMarching", "SkyRayMarching.lua", renderGraph.getRenderPass("RayMarchingPass"), 0);
        ...
        auto skyRayMarchingNode = renderNodes.emplace("skyRayMarchingNode", std::make_unique<RenderNode>()).first->second.get();
