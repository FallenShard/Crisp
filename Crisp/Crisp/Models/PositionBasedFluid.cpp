...
//         m_positions = std::make_unique<VulkanBuffer>(m_device, RendererConfig::VirtualFrameCount * vertexBufferSize,
...
//         m_colors = std::make_unique<VulkanBuffer>(m_device, RendererConfig::VirtualFrameCount * vertexBufferSize,
//             VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
//             VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
//         m_renderer->fillDeviceBuffer(m_colors.get(), colors.data(), vertexBufferSize, 0);
//
//         m_reorderedPositions = std::make_unique<VulkanBuffer>(m_device, RendererConfig::VirtualFrameCount *
//         vertexBufferSize,
...
//
//         m_cellCounts = std::make_unique<VulkanBuffer>(m_device, RendererConfig::VirtualFrameCount *
//         m_gridParams.numCells * sizeof(uint32_t),
//             VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
//         m_cellIndices = std::make_unique<VulkanBuffer>(m_device, RendererConfig::VirtualFrameCount * m_numParticles *
//         sizeof(uint32_t),
//             VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
//         m_indices = std::make_unique<VulkanBuffer>(m_device, RendererConfig::VirtualFrameCount * m_numParticles *
//         sizeof(uint32_t),
//             VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
//
//         m_blockSumRegionSize = static_cast<uint32_t>(std::max(m_gridParams.numCells / ScanElementsPerBlock *
//         sizeof(uint32_t), 32ull)); m_blockSums = std::make_unique<VulkanBuffer>(m_device,
//         RendererConfig::VirtualFrameCount * m_blockSumRegionSize,
//             VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
//
//         m_lambdas = std::make_unique<VulkanBuffer>(m_device, RendererConfig::VirtualFrameCount * m_numParticles *
//         sizeof(float),
//             VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
//         m_deltas = std::make_unique<VulkanBuffer>(m_device, RendererConfig::VirtualFrameCount * m_numParticles *
//         sizeof(glm::vec4),
//             VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
//         m_pressures = std::make_unique<VulkanBuffer>(m_device, RendererConfig::VirtualFrameCount * m_numParticles *
//         sizeof(float),
//             VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
//         m_velocities = std::make_unique<VulkanBuffer>(m_device, RendererConfig::VirtualFrameCount * m_numParticles *
//         sizeof(glm::vec4),
//             VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
//             VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
//         m_forces = std::make_unique<VulkanBuffer>(m_device, RendererConfig::VirtualFrameCount * m_numParticles *
//         sizeof(glm::vec4),
//             VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
//             VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
//         m_renderer->fillDeviceBuffer(m_forces.get(), forces.data(), vertexBufferSize, 0);
//
//         m_clearHashGridPipeline = createComputePipeline(m_renderer, "clear-hash-grid.comp", 1, 1, sizeof(uint32_t),
//         glm::uvec3(256, 1, 1)); m_clearGridDescGroup =
//         {
//             m_clearHashGridPipeline->allocateDescriptorSet(0)
//         };
//         m_clearGridDescGroup.postBufferUpdate(0, 0, { m_cellCounts->getHandle(), 0, m_gridParams.numCells *
//         sizeof(uint32_t) }); m_clearGridDescGroup.flushUpdates(m_device);
//
//         m_computeCellCountPipeline = createComputePipeline(m_renderer, "compute-cell-count.comp", 3, 1,
//         sizeof(glm::uvec3) + sizeof(float) + sizeof(uint32_t), glm::uvec3(256, 1, 1)); m_computeCellCountDescGroup =
//         {
...
//         m_computeCellCountDescGroup.postBufferUpdate(0, 1, { m_cellCounts->getHandle(), 0, m_gridParams.numCells *
//         sizeof(uint32_t) }); m_computeCellCountDescGroup.postBufferUpdate(0, 2, { m_cellIndices->getHandle(),    0,
//         m_numParticles * sizeof(uint32_t) }); m_computeCellCountDescGroup.flushUpdates(m_device);
//
//         // Scan for individual blocks
//         m_scanPipeline = createComputePipeline(m_renderer, "scan.comp", 2, 2, sizeof(int32_t) + sizeof(uint32_t),
//         glm::uvec3(256, 1, 1)); m_scanDescGroup =
//         {
//             m_scanPipeline->allocateDescriptorSet(0)
//         };
//         m_scanDescGroup.postBufferUpdate(0, 0, { m_cellCounts->getHandle(), 0, m_gridParams.numCells *
//         sizeof(uint32_t) }); m_scanDescGroup.postBufferUpdate(0, 1, { m_blockSums->getHandle(),  0,
//         m_blockSumRegionSize }); m_scanDescGroup.flushUpdates(m_device);
//
...
//         m_combineScanPipeline = createComputePipeline(m_renderer, "scan-combine.comp", 2, 1, sizeof(uint32_t),
//         glm::uvec3(512, 1, 1)); m_combineScanDescGroup =
//         {
//             m_combineScanPipeline->allocateDescriptorSet(0)
//         };
//         m_combineScanDescGroup.postBufferUpdate(0, 0, { m_cellCounts->getHandle(), 0, m_gridParams.numCells *
//         sizeof(uint32_t) }); m_combineScanDescGroup.postBufferUpdate(0, 1, { m_blockSums->getHandle(),  0,
//         m_blockSumRegionSize }); m_combineScanDescGroup.flushUpdates(m_device);
//
//         m_reindexPipeline = createComputePipeline(m_renderer, "reindex-particles.comp", 5, 1, sizeof(GridParams) +
//         sizeof(float), glm::uvec3(256, 1, 1)); m_reindexDescGroup =
//         {
//             m_reindexPipeline->allocateDescriptorSet(0)
//         };
//         m_reindexDescGroup.postBufferUpdate(0, 0, { m_positions->getHandle(),          0, m_numParticles *
//         sizeof(glm::vec4) }); m_reindexDescGroup.postBufferUpdate(0, 1, { m_cellCounts->getHandle(),         0,
//         m_gridParams.numCells * sizeof(uint32_t) }); m_reindexDescGroup.postBufferUpdate(0, 2, {
//         m_cellIndices->getHandle(),        0, m_numParticles * sizeof(uint32_t) });
//         m_reindexDescGroup.postBufferUpdate(0, 3, { m_indices->getHandle(),            0, m_numParticles *
//         sizeof(uint32_t) }); m_reindexDescGroup.postBufferUpdate(0, 4, { m_reorderedPositions->getHandle(), 0,
//         m_numParticles * sizeof(glm::vec4) }); m_reindexDescGroup.flushUpdates(m_device);
//
//         m_lambdasPipeline = createComputePipeline(m_renderer, "pbf-compute-lambdas.comp", 4, 1, sizeof(GridParams) +
//         sizeof(float), glm::uvec3(256, 1, 1)); m_lambdasDescGroup =
//         {
...
//         }); m_lambdasDescGroup.postBufferUpdate(0, 1, { m_cellCounts->getHandle(), 0, m_gridParams.numCells *
//         sizeof(uint32_t) }); m_lambdasDescGroup.postBufferUpdate(0, 2, { m_indices->getHandle(),    0, m_numParticles
//         * sizeof(uint32_t) }); m_lambdasDescGroup.postBufferUpdate(0, 3, { m_lambdas->getHandle(),    0,
//         m_numParticles * sizeof(float) }); m_lambdasDescGroup.flushUpdates(m_device);
//
//         m_deltasPipeline = createComputePipeline(m_renderer, "pbf-compute-deltas.comp", 5, 1, sizeof(GridParams) +
//         sizeof(float), glm::uvec3(256, 1, 1)); m_deltasDescGroup =
//         {
...
//         }); m_deltasDescGroup.postBufferUpdate(0, 1, { m_cellCounts->getHandle(), 0, m_gridParams.numCells *
//         sizeof(uint32_t) }); m_deltasDescGroup.postBufferUpdate(0, 2, { m_indices->getHandle(),    0, m_numParticles
//         * sizeof(uint32_t) }); m_deltasDescGroup.postBufferUpdate(0, 3, { m_lambdas->getHandle(),    0,
//         m_numParticles * sizeof(float) }); m_deltasDescGroup.postBufferUpdate(0, 4, { m_deltas->getHandle(),     0,
//         m_numParticles * sizeof(glm::vec4) }); m_deltasDescGroup.flushUpdates(m_device);
//
//         m_updatePosPipeline = createComputePipeline(m_renderer, "pbf-update-pos.comp", 2, 1, sizeof(uint32_t),
//         glm::uvec3(256, 1, 1)); m_updatePosDescGroup =
//         {
//             m_updatePosPipeline->allocateDescriptorSet(0)
//         };
//         m_updatePosDescGroup.postBufferUpdate(0, 0, { m_positions->getHandle(),  0, m_numParticles *
//         sizeof(glm::vec4)
//         }); m_updatePosDescGroup.postBufferUpdate(0, 1, { m_deltas->getHandle(),     0, m_numParticles *
//         sizeof(glm::vec4) }); m_updatePosDescGroup.flushUpdates(m_device);
//
//         m_forcesPipeline = createComputePipeline(m_renderer, "compute-forces.comp", 7, 1, sizeof(GridParams) +
//         sizeof(glm::uvec3) + sizeof(uint32_t) + 2 * sizeof(float), glm::uvec3(256, 1, 1)); m_forcesDescGroup =
//         {
//             m_forcesPipeline->allocateDescriptorSet(0)
//         };
//         m_forcesDescGroup.postBufferUpdate(0, 0, { m_positions->getHandle(),    0, vertexBufferSize });
//         m_forcesDescGroup.postBufferUpdate(0, 1, { m_cellCounts->getHandle(), 0, m_gridParams.numCells *
//         sizeof(uint32_t) }); m_forcesDescGroup.postBufferUpdate(0, 2, { m_indices->getHandle(),     0, m_numParticles
//         * sizeof(uint32_t) }); m_forcesDescGroup.postBufferUpdate(0, 3, { m_lambdas->getHandle(),   0, m_numParticles
//         * sizeof(float) }); m_forcesDescGroup.postBufferUpdate(0, 4, { m_pressures->getHandle(),  0, m_numParticles *
//         sizeof(float) }); m_forcesDescGroup.postBufferUpdate(0, 5, { m_velocities->getHandle(),  0, m_numParticles *
//         sizeof(glm::vec4) }); m_forcesDescGroup.postBufferUpdate(0, 6, { m_forces->getHandle(),    0, m_numParticles
//         * sizeof(glm::vec4) }); m_forcesDescGroup.flushUpdates(m_device);
//
//         m_predictPositionsPipeline = createComputePipeline(m_renderer, "pbf-predict.comp", 6, 1, sizeof(GridParams) +
//         sizeof(uint32_t) + sizeof(float), glm::uvec3(256, 1, 1)); m_predictPosDescGroup =
//         {
//             m_predictPositionsPipeline->allocateDescriptorSet(0)
//         };
//         m_predictPosDescGroup.postBufferUpdate(0, 0, { m_positions->getHandle(),    0, m_numParticles *
//         sizeof(glm::vec4) }); m_predictPosDescGroup.postBufferUpdate(0, 1, { m_velocities->getHandle(),  0,
//         m_numParticles * sizeof(glm::vec4) }); m_predictPosDescGroup.postBufferUpdate(0, 2, { m_forces->getHandle(),
//         0, m_numParticles * sizeof(glm::vec4) }); m_predictPosDescGroup.postBufferUpdate(0, 3, {
//         m_positions->getHandle(),    0, m_numParticles * sizeof(glm::vec4) });
//         m_predictPosDescGroup.postBufferUpdate(0, 4, { m_velocities->getHandle(),  0, m_numParticles *
//         sizeof(glm::vec4) }); m_predictPosDescGroup.postBufferUpdate(0, 5, { m_colors->getHandle(),     0,
//         m_numParticles * sizeof(glm::vec4) }); m_predictPosDescGroup.flushUpdates(m_device);
//     }
//
...
//         m_predictPositionsPipeline->setPushConstants(cmdBuffer, VK_SHADER_STAGE_COMPUTE_BIT, m_gridParams, timeDelta,
//         m_numParticles); m_predictPosDescGroup.setDynamicOffset(0, m_prevSection * m_numParticles *
//         sizeof(glm::vec4)); m_predictPosDescGroup.setDynamicOffset(1, m_prevSection * m_numParticles *
//         sizeof(glm::vec4)); m_predictPosDescGroup.setDynamicOffset(2, m_currentSection * m_numParticles *
//         sizeof(glm::vec4)); m_predictPosDescGroup.setDynamicOffset(3, m_currentSection * m_numParticles *
//         sizeof(glm::vec4)); m_predictPosDescGroup.setDynamicOffset(4, m_currentSection * m_numParticles *
//         sizeof(glm::vec4)); m_predictPosDescGroup.setDynamicOffset(5, m_currentSection * m_numParticles *
//         sizeof(glm::vec4)); m_predictPosDescGroup.bind<VK_PIPELINE_BIND_POINT_COMPUTE>(cmdBuffer,
//         m_predictPositionsPipeline->getPipelineLayout()->getHandle());
//
...
//         m_clearGridDescGroup.bind<VK_PIPELINE_BIND_POINT_COMPUTE>(cmdBuffer,
//         m_clearHashGridPipeline->getPipelineLayout()->getHandle());
//
...
//         m_computeCellCountPipeline->setPushConstant(cmdBuffer, VK_SHADER_STAGE_COMPUTE_BIT, sizeof(m_gridParams.dim),
//         m_gridParams.cellSize); m_computeCellCountPipeline->setPushConstant(cmdBuffer, VK_SHADER_STAGE_COMPUTE_BIT,
//         sizeof(m_gridParams.dim) + sizeof(m_gridParams.cellSize), m_numParticles);
...
//         m_computeCellCountDescGroup.bind<VK_PIPELINE_BIND_POINT_COMPUTE>(cmdBuffer,
//         m_computeCellCountPipeline->getPipelineLayout()->getHandle());
//
...
//         m_scanDescGroup.bind<VK_PIPELINE_BIND_POINT_COMPUTE>(cmdBuffer,
//         m_scanPipeline->getPipelineLayout()->getHandle());
//
//         glm::uvec3 numGroups = getNumWorkGroups(glm::uvec3(m_gridParams.numCells / ScanElementsPerThread, 1, 1),
//         *m_scanPipeline); vkCmdDispatch(cmdBuffer, numGroups.x, numGroups.y, numGroups.z);
//         insertComputeBarrier(cmdBuffer);
//
//         // Block-level scan
//         m_scanPipeline->setPushConstant(cmdBuffer, VK_SHADER_STAGE_COMPUTE_BIT, 0, 0);
//         m_scanPipeline->setPushConstant(cmdBuffer, VK_SHADER_STAGE_COMPUTE_BIT, 4, m_gridParams.numCells /
//         ScanElementsPerBlock); m_scanBlockDescGroup.setDynamicOffset(0, m_currentSection * m_blockSumRegionSize);
//         m_scanBlockDescGroup.setDynamicOffset(1, m_currentSection * m_blockSumRegionSize);
//         m_scanBlockDescGroup.bind<VK_PIPELINE_BIND_POINT_COMPUTE>(cmdBuffer,
//         m_scanPipeline->getPipelineLayout()->getHandle());
//
//         numGroups = getNumWorkGroups(glm::uvec3(m_gridParams.numCells / ScanElementsPerBlock / ScanElementsPerThread,
//         1, 1), *m_scanPipeline); vkCmdDispatch(cmdBuffer, numGroups.x, numGroups.y, numGroups.z);
//         insertComputeBarrier(cmdBuffer);
//
...
//         m_combineScanDescGroup.bind<VK_PIPELINE_BIND_POINT_COMPUTE>(cmdBuffer,
//         m_combineScanPipeline->getPipelineLayout()->getHandle());
//
...
//         m_reindexPipeline->setPushConstant(cmdBuffer, VK_SHADER_STAGE_COMPUTE_BIT, sizeof(m_gridParams),
//         m_numParticles); m_reindexDescGroup.setDynamicOffset(0, m_prevSection * m_numParticles * sizeof(glm::vec4));
...
//         m_reindexDescGroup.bind<VK_PIPELINE_BIND_POINT_COMPUTE>(cmdBuffer,
//         m_reindexPipeline->getPipelineLayout()->getHandle());
//
...
//         m_lambdasDescGroup.bind<VK_PIPELINE_BIND_POINT_COMPUTE>(cmdBuffer,
//         m_lambdasPipeline->getPipelineLayout()->getHandle());
//
...
//         m_deltasDescGroup.bind<VK_PIPELINE_BIND_POINT_COMPUTE>(cmdBuffer,
//         m_deltasPipeline->getPipelineLayout()->getHandle());
//
...
//         m_updatePosDescGroup.bind<VK_PIPELINE_BIND_POINT_COMPUTE>(cmdBuffer,
//         m_updatePosPipeline->getPipelineLayout()->getHandle());
//
...
//     }
//
//     std::vector<glm::vec4> PositionBasedFluid::createInitialPositions(glm::uvec3 fluidDim, float particleRadius)
//     const
//     {
