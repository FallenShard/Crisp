...
struct Literal : public Expression {
    Literal(TokenType type, std::any value)
        : type(type)
        , value(value) {}
...
struct ArraySpecifier : public Expression {
    ...
    ArraySpecifier(ExpressionPtr expr)
        : expr(std::move(expr)) {}
...
struct SubroutineQualifier : public TypeQualifier {
    SubroutineQualifier(Token name)
        : TypeQualifier(name) {}

    SubroutineQualifier(Token name, std::vector<ExpressionPtr>&& exprs)
        : TypeQualifier(name)
...
struct LayoutQualifier : public TypeQualifier {
    LayoutQualifier(Token name)
        : TypeQualifier(name) {}

    LayoutQualifier(Token name, std::vector<ExpressionPtr>&& exprs)
        : TypeQualifier(name)
        , ids(std::move(exprs)) {}
...
struct UnaryExpr : public Expression {
    UnaryExpr(Token op, ExpressionPtr expr)
        : op(op)
        , expr(std::move(expr)) {}
...
struct ListExpr : public Expression {
    ...
    ListExpr(std::string name)
        : name(name) {}

    ListExpr(std::string name, std::vector<ExpressionPtr>&& exprs)
        : name(name)
...
struct StructSpecifier : public TypeSpecifier {
    StructSpecifier(Token typeName)
