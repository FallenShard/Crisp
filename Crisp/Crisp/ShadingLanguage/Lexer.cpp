...
namespace crisp::sl
{
    ...
    void Lexer::scanToken()
    {
        ...
        case ':': addToken(TokenType::FullColon); break;
        ...
        case '#':
            while (peek() != '\n' && !isAtEnd())
                advance();
            break;
        case '&': addToken(match('&') ? TokenType::LogicalAnd : TokenType::BitwiseAnd); break;
        case '|': addToken(match('|') ? TokenType::LogicalOr  : TokenType::BitwiseOr); break;
        case '^': addToken(match('^') ? TokenType::LogicalXor : TokenType::BitwiseXor); break;
        case '!': addToken(match('=') ? TokenType::NotEqual : TokenType::ExclamationMark); break;
