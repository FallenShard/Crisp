...
void main() {
    ...
    if (model == kModelHomogeneousRgb) {
        const vec3 absorption = vec3(0.1f, 0.2f, 0.3f);
        const vec3 scattering = vec3(0.3f, 0.1f, 0.0f);
        const vec3 extinction = absorption + scattering;
        const float segmentDistance = 2.0f;
        const float distance = sampleHomogeneousMediumDistance(bsdfSample, extinction);
        const bool mediumEvent = distance < segmentDistance;
        const vec3 transmittance = homogeneousMediumTransmittance(
            extinction, mediumEvent ? distance : segmentDistance);
        const float distancePdf = homogeneousMediumDistancePdf(extinction, transmittance, mediumEvent);
        const vec3 weight = mediumEvent
            ? transmittance * scattering / distancePdf
    ...
    if (model == kModelMediumBounds) {
        const vec3 boundsMin = vec3(-1.0f);
        const vec3 boundsMax = vec3(1.0f);
        vec3 origin;
        vec3 direction;
        if (index % 4u == 0u) {
            origin = vec3(-3.0f, 0.0f, 0.0f);
            direction = vec3(1.0f, 0.0f, 0.0f);
        } else if (index % 4u == 1u) {
            origin = vec3(0.0f);
            direction = vec3(0.0f, 0.0f, 1.0f);
        } else if (index % 4u == 2u) {
            origin = vec3(2.0f, 0.0f, 0.0f);
        ...
        float entry;
        float exitDistance;
        const float maxDistance = index % 4u == 3u ? 2.5f : 10.0f;
        const bool hit = intersectMediumBounds(
            origin, direction, maxDistance, boundsMin, boundsMax, entry, exitDistance);
        results[index].value = vec4(entry, exitDistance, hit ? 1.0f : 0.0f, 0.0f);
        return;
    }
