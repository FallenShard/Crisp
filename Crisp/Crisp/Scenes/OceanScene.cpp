...
MainPCData values = { 0.0f, N, N, Lx, Lx, 40.0f, 40.0f, getNormVector(40, 40)[0], getNormVector(40, 40)[1],
    std::sqrt(40 * 40.0f + 40 * 40.0f), std::sqrt(40 * 40.0f + 40 * 40.0f) * std::sqrt(40 * 40.0f + 40 * 40.0f) / g,
    1000.0f, 1.0f };

std::unique_ptr<VulkanPipeline> createComputePipeline(Renderer* renderer, const glm::uvec3& workGroupSize,
    const PipelineLayoutBuilder& layoutBuilder, const std::string& shaderName)
{
...
std::unique_ptr<VulkanPipeline> createTrueFFTPipeline(Renderer* renderer, const glm::uvec3& workGroupSize,
...
std::unique_ptr<VulkanImage> createStorageImage(VulkanDevice& device, uint32_t layerCount, uint32_t width,
    ...
    , m_renderGraph(std::make_unique<RenderGraph>(renderer))
{
    ...
    m_resourceContext->addGeometry("ocean",
    ...
    m_renderer->enqueueResourceUpdate(
        [fftImg = fftImage.get(), this, dx = displacementXImage.get(), dz = displacementZImage.get(),
            nx = normalXImage.get(), nz = normalZImage.get()](VkCommandBuffer cmdBuffer)
        {
    ...
    oscillationPass.material->writeDescriptor(0, 0,
        m_resourceContext->getImageView("randImageView")->getDescriptorInfo(nullptr, VK_IMAGE_LAYOUT_GENERAL));
    oscillationPass.material->writeDescriptor(0, 1,
        m_resourceContext->getImageView("fftImageView0")->getDescriptorInfo(nullptr, VK_IMAGE_LAYOUT_GENERAL));
    oscillationPass.material->writeDescriptor(0, 2,
        m_resourceContext->getImageView("displacementXView0")->getDescriptorInfo(nullptr, VK_IMAGE_LAYOUT_GENERAL));
    oscillationPass.material->writeDescriptor(0, 3,
        m_resourceContext->getImageView("displacementZView0")->getDescriptorInfo(nullptr, VK_IMAGE_LAYOUT_GENERAL));
    oscillationPass.material->writeDescriptor(0, 4,
        m_resourceContext->getImageView("normalXView0")->getDescriptorInfo(nullptr, VK_IMAGE_LAYOUT_GENERAL));
    ...
    oscillationPass.preDispatchCallback = [this](VkCommandBuffer cmdBuffer, uint32_t /*frameIndex*/)
    {
        ...
        vkCmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 0,
    ...
    material->writeDescriptor(0, 1,
        m_resourceContext->getImageView("fftImageView" + std::to_string(layerToRead))
            ->getDescriptorInfo(m_resourceContext->getSampler("linearRepeat"), VK_IMAGE_LAYOUT_GENERAL));
    material->writeDescriptor(0, 2,
        m_resourceContext->getImageView("displacementXView" + std::to_string(ltr1))
            ->getDescriptorInfo(m_resourceContext->getSampler("linearRepeat"), VK_IMAGE_LAYOUT_GENERAL));
    material->writeDescriptor(0, 3,
        m_resourceContext->getImageView("displacementZView" + std::to_string(ltr2))
            ->getDescriptorInfo(m_resourceContext->getSampler("linearRepeat"), VK_IMAGE_LAYOUT_GENERAL));

    material->writeDescriptor(0, 4,
        m_resourceContext->getImageView("normalXView" + std::to_string(ln1))
            ->getDescriptorInfo(m_resourceContext->getSampler("linearRepeat"), VK_IMAGE_LAYOUT_GENERAL));
    material->writeDescriptor(0, 5,
        m_resourceContext->getImageView("normalZView" + std::to_string(ln2))
            ->getDescriptorInfo(m_resourceContext->getSampler("linearRepeat"), VK_IMAGE_LAYOUT_GENERAL));
    material->writeDescriptor(1, 0, *m_resourceContext->getUniformBuffer("camera"));
    material->writeDescriptor(1, 1, *m_resourceContext->getImageView("envIrrMap"),
        m_resourceContext->getSampler("linearClamp"));
    material->writeDescriptor(1, 2, *m_resourceContext->getImageView("filteredMap"),
        m_resourceContext->getSampler("linearMipmap"));
    material->writeDescriptor(1, 3, *m_resourceContext->getImageView("brdfLut"),
        m_resourceContext->getSampler("linearClamp"));
...
std::unique_ptr<VulkanImage> OceanScene::createInitialSpectrum()
{
    ...
    std::shared_ptr<VulkanBuffer> stagingBuffer = createStagingBuffer(m_renderer->getDevice(),
        initialSpectrum.size() * sizeof(glm::vec4), initialSpectrum.data());
    m_renderer->enqueueResourceUpdate(
        [img = image.get(), this, stagingBuffer](VkCommandBuffer cmdBuffer)
        {
            img->transitionLayout(cmdBuffer, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                VK_PIPELINE_STAGE_TRANSFER_BIT);

            img->copyFrom(cmdBuffer, *stagingBuffer, 0, 1);

            img->transitionLayout(cmdBuffer, VK_IMAGE_LAYOUT_GENERAL, VK_PIPELINE_STAGE_TRANSFER_BIT,
                VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT);
        });
...
int OceanScene::applyFFT(std::string image)
{
    ...
    int imageLayerWrite = 1;
    {
        ...
        bitReversePass.material->writeDescriptor(0, 0,
            m_resourceContext->getImageView(imageViewRead)->getDescriptorInfo(nullptr, VK_IMAGE_LAYOUT_GENERAL));
        ...
        m_renderGraph->addDependency(OscillationPass, image + "BitReversePass0",
            [this, image, imageLayerRead](const VulkanRenderPass&, VkCommandBuffer cmdBuffer, uint32_t /*frameIndex*/)
            {
    ...
    for (int i = 0; i < logN; ++i)
    {
        ...
        fftPass.material->writeDescriptor(0, 0,
            m_resourceContext->getImageView(imageViewRead)->getDescriptorInfo(nullptr, VK_IMAGE_LAYOUT_GENERAL));
        ...
        if (i == 0)
        {
            m_renderGraph->addDependency(image + "BitReversePass0", name,
                [this, image, imageLayerRead](const VulkanRenderPass&, VkCommandBuffer cmdBuffer,
                    uint32_t /*frameIndex*/)
                {
                    VkImageMemoryBarrier barrier = { VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER };
                    barrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
        ...
        if (i > 0)
        {
            ...
            m_renderGraph->addDependency(prevName, name,
                [this, image, imageLayerRead](const VulkanRenderPass&, VkCommandBuffer cmdBuffer,
                    uint32_t /*frameIndex*/)
                {
                    VkImageMemoryBarrier barrier = { VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER };
...

    {
        ...
        bitReversePass2.material->writeDescriptor(0, 0,
            m_resourceContext->getImageView(imageViewRead)->getDescriptorInfo(nullptr, VK_IMAGE_LAYOUT_GENERAL));
        ...
        m_renderGraph->addDependency(image + "TrueFFTPass" + std::to_string(logN - 1), image + "BitReversePass1",
            [this, image, imageLayerRead](const VulkanRenderPass&, VkCommandBuffer cmdBuffer, uint32_t /*frameIndex*/)
            {
                VkImageMemoryBarrier barrier = { VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER };
                barrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
    ...
    for (int i = 0; i < logN; ++i)
    {
        ...
        if (i == logN - 1)
        {
            ...
            m_renderGraph->addDependency(name, MainPass,
                [this, image, imageLayerWrite](const VulkanRenderPass&, VkCommandBuffer cmdBuffer,
                    uint32_t /*frameIndex*/)
                {
                    VkImageMemoryBarrier barrier = { VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER };
                    barrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
                    ...
                    vkCmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                        VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 1, &barrier);
                });
        }
        else if (i == 0)
        {
            m_renderGraph->addDependency(image + "BitReversePass1", name,
                [this, image, imageLayerRead](const VulkanRenderPass&, VkCommandBuffer cmdBuffer,
                    uint32_t /*frameIndex*/)
                {
                    VkImageMemoryBarrier barrier = { VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER };
