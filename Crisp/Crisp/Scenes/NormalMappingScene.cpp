...
void NormalMappingScene::createPlane()
{
    ...
    m_resourceContext->addImageWithView("normalMap",
        createTexture(m_renderer, "PbrMaterials/Hexstone/normal.png", VK_FORMAT_R8G8B8A8_UNORM));
    m_resourceContext->addImageWithView("diffuseMap",
        createTexture(m_renderer, "PbrMaterials/Hexstone/diffuse.png", VK_FORMAT_R8G8B8A8_SRGB));
    m_resourceContext->addImageWithView("specularMap",
        createTexture(m_renderer, "PbrMaterials/Hexstone/metallic.png", VK_FORMAT_R8G8B8A8_UNORM));
    ...
    material->writeDescriptor(1, 0, *m_resourceContext->getImageView("normalMap"),
        m_resourceContext->getSampler("linearRepeat"));
    material->writeDescriptor(1, 1, *m_resourceContext->getImageView("diffuseMap"),
        m_resourceContext->getSampler("linearRepeat"));
    material->writeDescriptor(1, 2, *m_resourceContext->getImageView("specularMap"),
        m_resourceContext->getSampler("linearRepeat"));
    ...
    // m_resourceContext->addGeometry("sphere", std::make_unique<Geometry>(m_renderer,
    ...
    // NANOSUIT
    TriangleMesh mesh(
        loadTriangleMesh(m_renderer->getResourcesPath() / "Meshes/nanosuit/nanosuit.obj", pbrVertexFormat));
    ...
    // nanosuit->transformPack->M = glm::rotate(glm::radians(0.0f), glm::vec3(1.0f, 0.0f, 0.0f)) *
    ...
    for (int i = 1; i < mesh.getViews().size(); ++i)
    {
        ...
        partMaterial->writeDescriptor(1, 0, *m_resourceContext->getImageView(normalMapKey),
            m_resourceContext->getSampler("linearRepeat"));
        partMaterial->writeDescriptor(1, 1, *m_resourceContext->getImageView(diffuseMapKey),
            m_resourceContext->getSampler("linearRepeat"));
        partMaterial->writeDescriptor(1, 2, *m_resourceContext->getImageView(specularMapKey),
            m_resourceContext->getSampler("linearRepeat"));
    ...
    for (int i = 0; i < 100; ++i)
    {
        for (int j = 0; j < 100; ++j)
        {
            ...
            for (int k = 1; k < mesh.getViews().size(); ++k)
                smallNano->pass(k, MainPass).material =
                    m_resourceContext->getMaterial("normalMapPart" + std::to_string(k));
        }
    }
