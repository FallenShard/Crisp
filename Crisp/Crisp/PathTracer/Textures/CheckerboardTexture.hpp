...
template <>
struct Default1<float> {
    static constexpr float value = 1.0f;
};

template <>
struct Default2<float> {
    static constexpr float value = 0.2f;
};

template <>
struct Default1<Spectrum> {
    static constexpr Spectrum value = Spectrum(1.0f, 0.5f, 0.5f);
};

template <>
struct Default2<Spectrum> {
    static constexpr Spectrum value = Spectrum(0.5f, 0.5f, 1.0f);
};
} // namespace internal
...
public:
    ...
    virtual T eval(const glm::vec2& uv) const override {
        ...
        if ((u > m_scale.x && v > m_scale.y) || (u < m_scale.x && v < m_scale.y)) {
            return m_value1;
        } else {
            return m_value2;
        }
