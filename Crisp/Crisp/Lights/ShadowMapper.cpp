...
ShadowMapper::ShadowMapper(Renderer* renderer, uint32_t numLights)
    ...
    , m_lightTransformBuffer(
          std::make_unique<UniformBuffer>(renderer, numLights * sizeof(glm::mat4), BufferUpdatePolicy::PerFrame))
    , m_lightFullTransforms(numLights)
    , m_lightFullTransformBuffer(
          std::make_unique<UniformBuffer>(renderer, numLights * sizeof(glm::mat4) * 2, BufferUpdatePolicy::PerFrame)) {}
...
void ShadowMapper::setLightTransform(uint32_t lightIndex, const glm::mat4& transform) {
    ...
    m_lightTransformBuffer->updateStagingBuffer3(m_lightTransforms.data(), m_lightTransforms.size() * sizeof(glm::mat4));
}
...
void ShadowMapper::setLightFullTransform(uint32_t lightIndex, const glm::mat4& view, const glm::mat4& projection) {
    ...
    m_lightFullTransformBuffer->updateStagingBuffer3(
        m_lightFullTransforms.data(), m_lightFullTransforms.size() * sizeof(glm::mat4));
}
