...
namespace crisp
{
    ...
    void LightSystem::update(const Camera& camera, float dt)
    {
        ...
        for (uint32_t i = 0; i < m_cascades.size(); ++i)
        {
            ...
            cascade.light->fitProjectionToFrustum(camera.computeFrustumPoints(cascade.begin, cascade.end), centerRadius, centerRadius.w, m_shadowMapSize);
    ...
    void LightSystem::updateSplitIntervals(const float zNear, const float zFar)
    {
        if (m_cascades.empty())
            return;
        ...
        {
            const float p = (i + 1) / static_cast<float>(m_cascades.size());
            const float logSplit = zNear * std::pow(ratio, p);
            const float linSplit = zNear + range * p;
            const float splitPos = m_splitLambda * (logSplit - linSplit) + linSplit;
