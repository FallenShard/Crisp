...
inline void initializeAccelerationStructureApi(VkDevice device)
{
    static bool initialized = false;
    if (initialized)
        return;

    GET_DEVICE_PROC_KHR(vkGetAccelerationStructureBuildSizes);
    GET_DEVICE_PROC_KHR(vkCreateAccelerationStructure);
    GET_DEVICE_PROC_KHR(vkCmdBuildAccelerationStructures);
    GET_DEVICE_PROC_KHR(vkGetAccelerationStructureDeviceAddress);
    GET_DEVICE_PROC_KHR(vkDestroyAccelerationStructure);
    initialized = true;
}
} // namespace

VulkanAccelerationStructure::VulkanAccelerationStructure(const VulkanDevice& device,
    const VkAccelerationStructureGeometryKHR& geometry, uint32_t primitiveCount, const glm::mat4& transform)
...
{
    initializeAccelerationStructureApi(device.getHandle());
    ...
    vkGetAccelerationStructureBuildSizes(device.getHandle(), VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
        &m_buildInfo, &m_primitiveCount, &m_buildSizesInfo);
    ...
    , m_buildSizesInfo{ VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_SIZES_INFO_KHR }
{
    ...
    m_instanceBuffer =
        std::make_unique<VulkanBuffer>(device, m_primitiveCount * sizeof(VkAccelerationStructureInstanceKHR),
...
VulkanAccelerationStructure::~VulkanAccelerationStructure()
{
    m_deallocator->deferDestruction(m_framesToLive, m_handle,
        ...
        {
            destroyDeferred(handle, deallocator, vkDestroyAccelerationStructure);
        });
}

void VulkanAccelerationStructure::build(const VulkanDevice& /*device*/, VkCommandBuffer cmdBuffer,
    const std::vector<VulkanAccelerationStructure*>& /*bottomLevelAccelStructures*/)
{
    if (m_buildInfo.type == VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR)
    {
        ...
        (void)(cmdBuffer);
        vkCmdBuildAccelerationStructures(cmdBuffer, 1, &m_buildInfo, rangeInfos.data());
    }
    else
    {
        VkAccelerationStructureBuildRangeInfoKHR rangeInfo;
        ...
        vkCmdUpdateBuffer(cmdBuffer, m_instanceBuffer->getHandle(), 0,
            m_instances.size() * sizeof(VkAccelerationStructureInstanceKHR), m_instances.data());
...
{
    m_accelerationStructureBuffer = std::make_unique<VulkanBuffer>(device, m_buildSizesInfo.accelerationStructureSize,
    ...
    vkCreateAccelerationStructure(device.getHandle(), &createInfo, nullptr, &m_handle);
    ...
    m_address = vkGetAccelerationStructureDeviceAddress(device.getHandle(), &addressInfo);

    m_scratchBuffer = std::make_unique<VulkanBuffer>(device, m_buildSizesInfo.buildScratchSize,
