    ...
    , m_drawComponent(parentForm->getRenderSystem())
{
    ...
    m_colorAnim = std::make_shared<PropertyAnimation<glm::vec4, Easing::SlowOut>>(0.5, color, color,
        [this](const glm::vec4& t)
        {
            setColor(t);
        });
    m_labelColorAnim =
        std::make_shared<PropertyAnimation<glm::vec4, Easing::SlowOut>>(0.5, glm::vec4(1.0f), glm::vec4(1.0f),
            [this](const glm::vec4& t)
            {
                m_label->setColor(t);
            });
