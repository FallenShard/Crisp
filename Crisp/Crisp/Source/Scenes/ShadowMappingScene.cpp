...
namespace crisp
{
        ...
        , m_device(m_renderer->getDevice())
    {
        ...
        m_renderPass = std::make_unique<SceneRenderPass>(m_renderer);
        m_renderer->setSceneImageView(m_renderPass->createRenderTargetView(0, Renderer::NumVirtualFrames));
        ...
        m_colorPipeline = createColorPipeline(m_renderer, m_renderPass.get());
        m_colorMaterial = std::make_unique<Material>(m_colorPipeline.get(), std::vector<uint32_t>{ 0 });
        m_device->postDescriptorWrite(m_colorMaterial->makeDescriptorWrite(0, 0), m_transformsBuffer->getDescriptorInfo(0, sizeof(TransformPack)));

        auto imageBuffer = std::make_unique<ImageFileBuffer>("Resources/Textures/normalmap.jpg");
        VkExtent3D imageExtent = { imageBuffer->getWidth(), imageBuffer->getHeight(), 1u };
        uint32_t mipLevels = imageBuffer->getMipLevels();
        ...
        m_normalMap->fill(imageBuffer->getData(), imageBuffer->getByteSize());
        ...
        m_normalMapMaterial = std::make_unique<Material>(m_normalMapPipeline.get(), std::vector<uint32_t>{ 0, 1 });
        ...
        m_device->postDescriptorWrite(m_normalMapMaterial->makeDescriptorWrite(0, 0), m_transformsBuffer->getDescriptorInfo(0, sizeof(TransformPack)));
        ...
        m_sphereGeometry = makeGeometry(m_renderer, "sphere.obj", { VertexAttribute::Position });
        m_planeGeometry  = makeGeometry(m_renderer, makePlaneMesh({ VertexAttribute::Position, VertexAttribute::Normal, VertexAttribute::TexCoord, VertexAttribute::Tangent }));
    ...
    void ShadowMappingScene::render()
    {
        ...
        m_renderer->enqueueDrawCommand([this](VkCommandBuffer cmdBuffer)
        {
            ...
            m_renderPass->begin(cmdBuffer);

            m_colorPipeline->bind(cmdBuffer);
            m_renderer->setDefaultScissor(cmdBuffer);
            m_renderer->setDefaultViewport(cmdBuffer);
            m_colorMaterial->setDynamicOffset(frameIdx, 0, m_transformsBuffer->getDynamicOffset(frameIdx) + 0 * sizeof(TransformPack));
            m_colorMaterial->bind(frameIdx, cmdBuffer);
            m_colorPipeline->setPushConstant(cmdBuffer, VK_SHADER_STAGE_FRAGMENT_BIT, 0, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
            m_sphereGeometry->bindAndDraw(cmdBuffer);

            m_normalMapPipeline->bind(cmdBuffer);
            m_renderer->setDefaultScissor(cmdBuffer);
            m_renderer->setDefaultViewport(cmdBuffer);
            ////m_normalMapPipeline->setPushConstant(cmdBuffer, VK_SHADER_STAGE_FRAGMENT_BIT, 0, m_vsmLightTransforms[0]);
            ////m_normalMapPipeline->setPushConstant(cmdBuffer, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(glm::mat4), m_vsmLightTransforms[1]);
            //m_normalMapMaterial->setDynamicOffset(frameIdx, 2, m_cameraBuffer->getDynamicOffset(frameIdx));
            m_normalMapMaterial->setDynamicOffset(frameIdx, 0, m_transformsBuffer->getDynamicOffset(frameIdx) + 1 * sizeof(TransformPack));
            m_normalMapMaterial->setDynamicOffset(frameIdx, 1, m_cameraBuffer->getDynamicOffset(frameIdx));
            m_normalMapMaterial->bind(frameIdx, cmdBuffer);
            m_planeGeometry->bindAndDraw(cmdBuffer);
            //m_planeGeometry->draw(cmdBuffer);

            m_renderPass->end(cmdBuffer);

            m_renderPass->getRenderTarget(0)->transitionLayout(cmdBuffer, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, frameIdx, 1,
        ...
        //    //m_treeBaseGeometry->bindAndDraw(commandBuffer);
        //    m_treeBaseGeometry->bindAndDraw(commandBuffer);
        //    //m_sphereGeometry->bindGeometryBuffers(commandBuffer);
        //    //m_sphereGeometry->draw(commandBuffer);
        //
        ...
        //    m_shadowMapDescGroup.bind(commandBuffer, m_shadowMapPipeline->getPipelineLayout());
        //    m_cwPlaneGeometry->bind(commandBuffer);
        ...
        //    //m_sphereGeometry->draw(commandBuffer);
        //
        //    m_cubeGeometry->bind(commandBuffer);
        //    for (int i = 2; i < 22; i++)
        //    {
        ...
        //    m_normalMapMaterial->setDynamicOffset(frameIdx, 2, m_cameraBuffer->getDynamicOffset(frameIdx));
        //    m_normalMapMaterial->setDynamicOffset(frameIdx, 0, m_transformsBuffer->getDynamicOffset(frameIdx));
        //    m_normalMapMaterial->bind(frameIdx, commandBuffer);
        //    m_planeGeometry->bind(commandBuffer);
