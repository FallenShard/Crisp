...
namespace crisp
{
    std::unique_ptr<VulkanPipeline> createPbrPipeline(Renderer* renderer, VulkanRenderPass* renderPass, unsigned int numMaterials)
    {
        ...
        auto descPool = createDescriptorPool(device->getHandle(), layoutBuilder, { numMaterials, numMaterials * Renderer::NumVirtualFrames }, numMaterials * (1 + Renderer::NumVirtualFrames));
