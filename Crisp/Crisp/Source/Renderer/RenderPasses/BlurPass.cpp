...
namespace crisp
{
    ...
    void BlurPass::begin(VkCommandBuffer cmdBuffer) const
    {
    ...
    VkImage BlurPass::getColorAttachment(unsigned int index) const
    {
        return m_renderTargets[index]->getImage()->getHandle();
    }

    VkImageView BlurPass::getAttachmentView(unsigned int index, unsigned int frameIndex) const
    {
        return m_renderTargetViews.at(index).at(frameIndex)->getHandle();
    }

    VkFormat BlurPass::getColorFormat() const
    {
        return m_colorFormat;
    }

    VkFramebuffer BlurPass::getFramebuffer(unsigned int index) const
    {
        return m_framebuffers[index];
    }

    std::unique_ptr<TextureView> BlurPass::createRenderTargetView(unsigned int index) const
    {
