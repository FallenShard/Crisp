...
layout(set = 0, binding = 1) uniform LightTransforms
{
    ...
    mat4 lightView1;
    mat4 lightProjection1;
	mat4 padding1;
	mat4 lightViewProjection2;
    mat4 lightView2;
    mat4 lightProjection2;
	mat4 padding2;
};
...
layout(set = 1, binding = 0) uniform sampler2D shadowMaps[3];
...
{
	vec4 lsPos = lightViewProjection * vec4(worldPos, 1.0f);
    vec3 projPos = lsPos.xyz;// / lsPos.w;
...
void main()
{
    ...
    if (eyeDist > 45.0f)
	{
		lvp = lightViewProjection2;
		sam = 2;
	}
	else if (eyeDist > 15.0f)
	{
		lvp = lightViewProjection1;
		sam = 1;
	}
	else
	{
		lvp = lightViewProjection;
		sam = 0;
	}

	vec3 V = shadowEstimate(worldPos, bias, shadowMaps[sam], lvp);
