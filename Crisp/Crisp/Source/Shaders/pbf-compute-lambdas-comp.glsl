...
void main()
{
    ...
    float density = 0.0f;
    vec3 gradPiCi = vec3(0.0f);
    float sumGradCi2 = 0.0f;
        ...
        for (uint cellY = lo.y; cellY <= hi.y; cellY++)
        {
            for (uint cellX = lo.x; cellX <= hi.x; cellX++)
            {
                ...
                for (uint k = cellStart; k < cellEnd; k++)
                {
                    ...
                    if (dist > 0.0f && dist < h)
                    {
                        vec3 gradPkCi = spikyGrad(diff, dist);
                        gradPkCi *= invRestDensity;
                        sumGradCi2 += dot(gradPkCi, gradPkCi);
    ...
    sumGradCi2 += dot(gradPiCi, gradPiCi);

    float Ci = density * invRestDensity - 1.0f;

    float lambda = -Ci / (sumGradCi2 + 0.00001f);

    lambdas[threadIdx] = lambda;
}