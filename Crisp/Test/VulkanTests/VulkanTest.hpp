...
inline auto createDevice()
{
    struct
    {
        ...
        std::unique_ptr<crisp::VulkanContext> context{ std::make_unique<crisp::VulkanContext>(
            window->createSurfaceCallback(), crisp::ApplicationEnvironment::getRequiredVulkanExtensions(), true) };
    ...
    auto device = std::make_unique<crisp::VulkanDevice>(*deps.physicalDevice,
        crisp::createDefaultQueueConfiguration(*deps.context, *deps.physicalDevice), 2);
