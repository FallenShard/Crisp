...
namespace crisp
{
    ...
    Spectrum EnvironmentLight::sample(Light::Sample& sample, Sampler& sampler) const
    {
        ...
        Spectrum val1 = m_probe->fetch(xPos, yPos) * dx2 * dy2
                      + m_probe->fetch(xPos + 1, yPos) * dx1 * dy2;
        Spectrum val2 = m_probe->fetch(xPos, yPos + 1) * dx2 * dy1
                      + m_probe->fetch(xPos + 1, yPos + 1) * dx1 * dy1;
