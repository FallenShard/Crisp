...
// const float AP_SLICE_COUNT = 32.0f;
// const float AP_KM_PER_SLICE = 4.0f;
...
// {
// 	vec4 Luminance		: SV_TARGET0;
// //#if COLORED_TRANSMITTANCE_ENABLED
// 	vec4 Transmittance	: SV_TARGET1;
// //#endif
// };
...
// #if COLORED_TRANSMITTANCE_ENABLED
// 	output.Transmittance = vec4(0, 0, 0, 1);
// #endif

// 	vec2 pixPos = Input.position.xy;
// 	AtmosphereParameters Atmosphere = GetAtmosphereParameters();

// 	vec3 ClipSpace = vec3((pixPos / vec2(gResolution))*vec2(2.0, -2.0) - vec2(1.0, -1.0), 1.0);
// 	vec4 HPos = mul(gSkyInvViewProjMat, vec4(ClipSpace, 1.0));
...
// 		output.Luminance = vec4(SkyViewLutTexture.SampleLevel(samplerLinearClamp, uv, 0).rgb + GetSunLuminance(WorldPos, WorldDir, Atmosphere.BottomRadius), 1.0);
// 		return output;
// 	}
// #else
...
// 	const vec4 AP = Weight * AtmosphereCameraScatteringVolume.SampleLevel(samplerLinearClamp, vec3(pixPos / vec2(gResolution), w), 0);
// 	L.rgb += AP.rgb;
// 	float Opacity = AP.a;

// 	output.Luminance = vec4(L, Opacity);
...
// 		output.Luminance = vec4(GetSunLuminance(WorldPos, WorldDir, Atmosphere.BottomRadius), 1.0);
// 		return output;
// 	}
...
// 	output.Luminance = vec4(L, 1.0 - Transmittance);
// #endif
...
void main()
{
	...
	vec3 sunDir = vec3(0.0, sqrt(clamp(1.0 - cosSunZenithAngle * cosSunZenithAngle, 0, 1)), cosSunZenithAngle);
...
// vec4 RenderTransmittanceLutPS(VertexOutput Input) : SV_TARGET
// {
// 	vec2 pixPos = Input.position.xy;
// 	AtmosphereParameters Atmosphere = GetAtmosphereParameters();
...
// vec4 RenderCameraVolumePS(GeometryOutput Input) : SV_TARGET0
// {
// 	vec2 pixPos = Input.position.xy;
// 	AtmosphereParameters Atmosphere = GetAtmosphereParameters();
